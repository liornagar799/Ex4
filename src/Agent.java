...
//    }
    ...
    public int findNearPokpath(LinkedList<Pokemon> poks, DirectedWeightedGraphAlgorithms ga) {
        Pokemon chosen = poks.contains(this.gointTo) ? this.gointTo : null;
        int tempNextNode = 0;
        int nextNode;
        double minEff = Double.MAX_VALUE;
        double tempEff;
...
//                this.setDest(nn);
            ...
            currDist =  ga.getGraph().getNode(nn).getWeight()+filteredList.get(i).get_edge().getWeight();
